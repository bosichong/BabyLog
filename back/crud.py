...
def create_data(db: Session):
    ...
    if get_users_count(db) == 0:
        add_user(db, User(username='baba', hashed_password=hashed_password, familymember='爸爸'))
        add_user(db, User(username='mama', hashed_password=hashed_password, sex='0', familymember='妈妈'))
        logger.info("创建超级管理员：baba")
    user = get_user_by_username(db, "baba")
    ...
    if get_casbin_rule_count(db) <= 0:
        ...
        create_casbin_rule_g(db, CasbinRule(ptype='g', v0='baba', v1=role_superadmin.role_key))
        create_casbin_rule_g(db, CasbinRule(ptype='g', v0='mama', v1=role_superadmin.role_key))
...
def create_babys(db: Session):
    create_baby(db, '大宝', datetime.date(2018, 5, 22))
    create_baby(db, '二宝', datetime.date(2020, 4, 12))
...
def get_baby_by_name(db: Session, name: str):
    return db.query(Baby).filter_by(name=name).first()


def delete_baby_by_id(db: Session, baby_id: int):
    baby = db.query(Baby).filter_by(id=baby_id).first()
    bbs = get_bb_by_baby_id(db, baby.id)
    for b in bbs:
        db.delete(b)
        db.commit()
    try:
...
def create_blog(db: Session, content: str, user_id: int):
    try:
        blog = Blog()
        blog.blog = content
...
def get_blogs(db: Session, skip: int = 0, limit: int = 10, keyword=''):
    return db.query(Blog).order_by(Blog.create_time.desc()).filter(Blog.blog.like("%" + keyword + "%")).offset(skip).limit(limit).all()


def get_blogs_count_by_keyword(db: Session, keyword: str):
...
def create_photo(db: Session, file_name: str, file_path: str, file_url: str, user_id: int, blog_id: int):
    try:
        photo = Photo(file_name=file_name, file_path=file_path, file_url=file_url, user_id=user_id, blog_id=blog_id)
        db.add(photo)
        db.commit()
        return photo
    except Exception as e:
...
def delete_photo_by_id(db: Session, id: int):
...
def get_healthys(db: Session):
    return db.query(Healthy).order_by(Healthy.create_time.desc()).all()


def get_healthys1(db: Session):
    return db.query(Healthy).all()


def get_healthys_by_babyid(db: Session, baby_id: int):
    return db.query(Healthy).filter_by(baby_id=baby_id).all()


def get_healthy_by_id(db: Session, id: int):
    return db.query(Healthy).filter_by(id=id).first()
...
def delete_healthy_by_id(db: Session, id: int):
    try:
        hl = get_healthy_by_id(db, id)
        db.delete(hl)
        db.commit()
        return True
    except Exception as e:
...
def get_bb_by_blog_id(db: Session, blog_id: int):
    return db.query(blog_baby).filter_by(blog_id=blog_id).all()


def get_bb_by_baby_id(db: Session, baby_id: int):
    return db.query(blog_baby).filter_by(baby_id=baby_id).all()


def get_bb_count_by_baby_id(db: Session, baby_id: int):
    return db.query(blog_baby).filter_by(baby_id=baby_id).count()


def get_bb_by_id(db: Session, id: int):
    return db.query(blog_baby).filter_by(id=id).first()


def delete_bb_by_id(db: Session, id: int):
    bb = get_bb_by_id(db, id)
    if bb:
        db.delete(bb)
        db.commit()
...
def create_user(db: Session, username: str, password: str, sex: str, familymember: str):
    """
    创建一个新用户
    :param db:
    :param username:
